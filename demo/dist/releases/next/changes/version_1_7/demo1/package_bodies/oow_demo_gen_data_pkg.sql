...
-- changeset DEMO1:1770123135931 stripComments:false  logicalFilePath:version_1_7/demo1/package_bodies/oow_demo_gen_data_pkg.sql
-- sqlcl_snapshot demo/src/database/demo1/package_bodies/oow_demo_gen_data_pkg.sql:null:47f8bea39953c2e014d26064953a0dd364c2bf74:create

create or replace package body demo1.oow_demo_gen_data_pkg is

    g_last_history_id number;

    function generate_transaction return number is
    begin
...
--
    ...
    procedure oow_demo_gen_sales_data (
        ...
        select
            owner,
            start_time,
            to_char(start_time, 'YYYY.MM.DD HH24:MI:SS') start_txt
        from
    ...
    --
        g_progress := 3;
        insert into oow_demo_hist_gen_log (
            owner,
            start_time,
            num_days,
            max_orders
                   ...
                   localtimestamp,
                   p_days,
                   p_orders ) returning id into l_log_id;
    ...
    --
        ...
        --
            ...
            --
                for l_order in 1..greatest(
                    round(dbms_random.value(0, p_orders)),
                    1
                ) loop
                    g_progress := 7;
                    g_context := 'i='
                ...
                -- something unique for a transaction id
                    l_transaction := to_char(l_date, 'YYYYMMDD')
                                     || '-'
                                     || l_order;
                ...
                --
                    for p in (
                        select
                            ...
                            item_price,
                            case
                                when l_date between sale_start_date and sale_end_date then
                                    3
                                else
                                    1
                            end weight
                    ...
                    -- for one transaction orders buy 1 anyway
                        ...
                        g_price := null;
                        g_progress := 8;
                        if ( round(dbms_random.value(0, p.weight)) != 0 )
                        or (
                            p_days = 1
                            and p_orders = 1
                        ) then
                        ...
                        -- order it up
                            ...
                            insert into oow_demo_sales_history (
                                ...
                                item_price
                                       ...
                                       l_qty,
                                       l_transaction,
                                       l_item_price ) returning id into g_last_history_id;

                            g_insert_count := g_insert_count + 1;
                            if
                                l_max_rows != 0
                                and g_insert_count >= nvl(l_max_rows, 5000)
                            then
                                l_continue := false;
                            end if;
    ...
    procedure exec_100_transactions is
        x       number;
        l_found boolean;
    begin
        for i in 1..100 loop
